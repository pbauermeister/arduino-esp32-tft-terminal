...
`arduino-esp32-tft-terminal` is a gadget sporting a small 240×135 colour TFT display, enclosed in a 3D-printed case, driven by a Python program running on the computer (Linux) over USB.
It provides extensible apps — performance monitors (local/remote), games (asteroids), physics demos (collisions, bubbles), and graphical demos (3D cube, starfield, tunnel).

The split of concerns is fixed by a three-part architecture:

- **Board (server)** — a dumb graphical server: exposes TFT drawing primitives and button readout over a USB text protocol; holds no app logic.
- **Computer (client)** — owns all app logic; drives the board by sending command lines and reading answers.
- **Protocol** — the contract between them; the board maps most commands directly to TFT library calls.

When adding capability, place it by this split — pixels and buttons on the board, behaviour on the computer — not by what is locally convenient.
The protocol is specified in `README-protocol.md`; flicker-avoidance techniques in `README-animations.md`.
...
- [README-STATE-DETECTION.md](https://github.com/pbauermeister/claude-busy-monitor/blob/main/README-STATE-DETECTION.md) — design notes for the `claude_mon` app's session-state classifier (lives in the `claude-busy-monitor` project)
